...
export async function POST(req: Request) {
    const limited = await rateLimit(getClientIp(req), 'structure-note:post', { limit: 10 });
    ...
    try {
        ...
        const userLimited = await rateLimitByUser(user.id, 'structure-note:post', { limit: 10 });
